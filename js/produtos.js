...
document.getElementById("formProduto")?.addEventListener("submit", async function(e) {
  ...
  if (!nome) { document.getElementById("erro-nome").textContent = "Digite o nome do produto."; return; }
  document.getElementById("erro-nome").textContent = "";

  if (isNaN(quantidade) || quantidade < 0 || quantidade > 10000) { document.getElementById("erro-quantidade").textContent = "Quantidade inválida."; return; }
  document.getElementById("erro-quantidade").textContent = "";

  if (isNaN(preco) || preco <= 0 || preco > 10000) { document.getElementById("erro-preco").textContent = "Preço inválido."; return; }
...
async function comprarProduto() {
  ...
  const produto = estoque.find(p => p.nome.toLowerCase() === nome.toLowerCase());
  if (produto && qtd > 0) {
    await updateDoc(doc(db, "produtos", produto.id), { quantidade: produto.quantidade + qtd });
    carregarProdutos();
...
async function editarPreco() {
  ...
  if (produto && novoPreco > 0) {
    await updateDoc(doc(db, "produtos", produto.id), { preco: novoPreco });
    carregarProdutos();
